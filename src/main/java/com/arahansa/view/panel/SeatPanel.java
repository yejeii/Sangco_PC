...
public class SeatPanel extends JPanel {
	...
	public static void main(String[] args) {
		...
		frameTest.add(new SeatPanel(1));
		frameTest.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frameTest.setSize(99, 144);
		frameTest.setVisible(true);
	}
	
	public SeatPanel(int numSeat) {
	...
	private void init() {
		...
		panLayered.add(panImg, 0, 0);
		panLayered.add(panContent, 1, 0);
		add(panLayered);
	...
	public void img(String filename) {
		...
		try {
			img = ImageIO.read(new File("src/main/resources/img/" + filename + ".png"));
		} catch (IOException e) {
